...
function ImportPrepTab() {
  const [masterData, setMasterData] = useState([]); // [{name, employmentType, department}]
            ...
            body: JSON.stringify({
              ...
              messages: [{
                ...
                content: [
                  ...
                  {
                    type: "text",
                    text: `この請求書PDFから以下の情報を抽出してください。
必ずJSON形式のみで返してください。前置きや説明は不要です。
{
  "name": "氏名または外注先名（敬称・法人格を除く）",
  "amount": 請求金額（税込の数値のみ、カンマや円記号なし）
}
複数人分ある場合はJSON配列で返してください。
最終的な支払い合計額を優先してください。`
                  }
                ]
              }]
    ...
    for (const file of files) {
      ...
      items.forEach(item => {
        ...
        newRows.push({
          ...
          criteriaValue: match?.employmentType === "社内外注" ? taxExcluded : (match?.employmentType === "派遣社員" ? taxExcluded : amount),
          matched: !!match,
          fileName: file.name,
        });
      });

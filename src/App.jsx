...
function parsePdfText(text, masterData) {
  ...
  // 方針1: マスタ氏名がテキストに含まれる行を探す
  if (masterNames.length > 0) {
    masterNames.forEach(master => {
      // 氏名が含まれる行を探す
      const nameVariants = [master.name, master.norm];
      let foundLine = -1;
      lines.forEach((line, idx) => {
        const normLine = normalizeSpaces(line);
        if (nameVariants.some(v => normLine.includes(normalizeSpaces(v)))) {
          foundLine = idx;
        }
      });
      ...
      if (foundLine >= 0) {
        // 同じ行か近傍行から金額を探す
        let amount = 0;
        const searchRange = lines.slice(Math.max(0, foundLine - 1), foundLine + 4);
        for (const l of searchRange) {
          const nums = l.match(/[\d,，]{4,}/g);
          if (nums) {
            const candidates = nums.map(n => parseInt(n.replace(/[,，]/g, ""), 10)).filter(n => n > 1000);
            if (candidates.length > 0) {
              amount = candidates[candidates.length - 1]; // 末尾の数値（金額らしい）
              break;
            }
          }
        }
        if (amount > 0) {
          results.push({ name: master.name, amount, matched: true, master });
        }
      }
    });
  }

  // 方針2: マスタ照合できなかった場合 or マスタなしの場合
  // 「氏名パターン」＋「金額」を全文から抽出
  if (results.length === 0) {
    // 日本語氏名パターン（漢字2〜4文字 + スペース + 漢字1〜3文字）
    const namePattern = /[\u4e00-\u9fa5ぁ-んァ-ヶ]{1,4}[\s\u3000][\u4e00-\u9fa5ぁ-んァ-ヶ]{1,4}/g;
    lines.forEach((line, idx) => {
      ...
      if (nameMatches) {
        nameMatches.forEach(rawName => {
          ...
          // 近傍から金額を探す
          const searchRange = lines.slice(idx, Math.min(lines.length, idx + 4));
          let amount = 0;
          for (const l of searchRange) {
            const nums = l.match(/[\d,，]{4,}/g);
            if (nums) {
          ...
          if (amount > 0) {
            results.push({ name, amount, matched: false, master: null });
          }
        });
      }
    });

    // 方針3: 氏名が見つからない場合、御請求金額・税込金額キーワードで金額だけ拾う
    if (results.length === 0) {
      const amountKeywords = /御請求金額|ご請求金額|請求金額|税込|合計|総合計/;
      lines.forEach(line => {
        if (amountKeywords.test(line)) {
          ...
          if (nums) {
            const candidates = nums.map(n => parseInt(n.replace(/[,，]/g, ""), 10)).filter(n => n > 1000);
            if (candidates.length > 0) {
              results.push({ name: "", amount: candidates[candidates.length - 1], matched: false, master: null });
            }
          }
        }
      });
...
function ImportPrepTab() {
  ...
  const handleMasterImport = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      setMasterData(data.map(r => ({
        name: String(r["氏名"] || ""),
        employmentType: String(r["雇用形態"] || ""),
        department: String(r["部門"] || r["部門1"] || ""),
      })).filter(r => r.name));
      setStatus(`マスタ読込完了: ${data.length}件`);
    };
    reader.readAsBinaryString(file);
    e.target.value = "";
  };
  ...
  const handlePdfImport = async (e) => {
    ...
    for (const file of files) {
      ...
      parsed.forEach(item => {
        ...
        const isTaxIncluded = match?.employmentType === "社内外注";
        const criteriaValue = isTaxIncluded ? Math.round(amount / 1.1) : amount;
        newRows.push({
          id: Math.random().toString(36).slice(2),
          extractedName,
          amount,
  ...
  const updateRow = (id, field, value) => {
    setRows(prev => prev.map(r => {
      ...
      if (field === "amount") {
        ...
        if (updated.employmentType === "社内外注" || updated.employmentType === "派遣社員") {
          updated.criteriaValue = Math.round(amt / 1.1);
        }
      }
      return updated;
    }));
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="border border-gray-200 rounded-xl p-4 bg-white shadow-sm">
        ...
        <p className="text-sm text-gray-500 mb-3">氏名・雇用形態・部門が記載されたExcelファイルをアップロードしてください。</p>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="px-3 py-2 text-left border-b font-semibold text-gray-600">税込金額</th>
                  <th className="px-3 py-2 text-left border-b font-semibold text-gray-600">雇用形態</th>
                  <th className="px-3 py-2 text-left border-b font-semibold text-gray-600">判定基準値（税抜）</th>
              ...
              <tbody>
                {rows.map(row => (
                  <tr key={row.id} className={row.matched ? "border-b border-gray-100" : "border-b border-red-200 bg-red-50"}>
                    ...
                    <td className="px-3 py-2">
                      {(row.employmentType === "社内外注" || row.employmentType === "派遣社員") ? (
                        <span className="text-indigo-700 font-medium">¥{fmt(row.criteriaValue)}</span>
                      ) : (
